...
class TestCreateStudentUseCase:
    ...
    def test_create_student_success(self):
        ...
        dto = CreateStudentDTO("Carlos", "Gómez", 20, "3001234567", "carlos@example.com", "activo")
    ...
    def test_create_student_duplicate_email_raises_error(self):
        mock_repo = MagicMock(spec=StudentRepositoryPort)
        existing_student = Student(id=1, nombres="Existe", apellidos="Ya", edad=25, telefono="3001234567", email="carlos@example.com")
...
class TestGetStudentUseCase:
    ...
    def test_get_student_success(self):
        mock_repo = MagicMock(spec=StudentRepositoryPort)
        student = Student(id=5, nombres="Marta", apellidos="López", edad=21, telefono="3101234567", email="marta@example.com")
...
class TestListStudentsUseCase:
    ...
    def test_list_students_paginated(self):
        mock_repo = MagicMock(spec=StudentRepositoryPort)
        students = [
            Student(id=1, nombres="A", apellidos="B", edad=20, telefono="3001234567", email="a@a.com"),
            Student(id=2, nombres="C", apellidos="D", edad=22, telefono="3007654321", email="c@c.com"),
        ]
...
class TestUpdateStudentUseCase:
    ...
    def test_update_student_success(self):
        mock_repo = MagicMock(spec=StudentRepositoryPort)
        original = Student(id=1, nombres="Juan", apellidos="Pérez", edad=20, telefono="3001234567", email="juan@example.com", status="activo")
    ...
    def test_update_student_email_conflict(self):
        mock_repo = MagicMock(spec=StudentRepositoryPort)
        original = Student(id=1, nombres="Juan", apellidos="Pérez", edad=20, telefono="3001234567", email="juan@example.com")
        another = Student(id=2, nombres="María", apellidos="Gómez", edad=22, telefono="3101234567", email="maria@example.com")
        mock_repo.get_by_id.return_value = original
...
class TestDeleteStudentUseCase:
    ...
    def test_delete_student_success(self):
        mock_repo = MagicMock(spec=StudentRepositoryPort)
        student = Student(id=3, nombres="Andrés", apellidos="Diaz", edad=24, telefono="3001112233", email="andres@test.com")
        mock_repo.get_by_id.return_value = student
